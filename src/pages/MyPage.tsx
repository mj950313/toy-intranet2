...
const MyPage: React.FC = () => {
  ...
  const userSchedulesForWeek = loginUser
    ? filterSchedulesByWeek(loginUser.schedulesByDate)
    : [];
          ...
          <tbody>
            {Array.from({ length: 24 }).map((_, hourIndex) => (
              <tr key={hourIndex}>
                ...
                {userSchedulesForWeek.map((scheduleByDate) => {
                  const scheduleForHour = scheduleByDate.schedules.find(
                    (schedule) => {
                      const [startHour] = schedule.time
                        .split("-")
                        .map((time) => parseInt(time));
                      ...
                      {scheduleForHour ? (
                        <div>
                          <span className="font-semibold">
                            {scheduleForHour.time}
                          </span>{" "}
                          - {scheduleForHour.title}
                        </div>
                      ) : null}
                    </td>
                  );
          ...
          <div key={loginUser.id} className="mb-4">
            ...
            <p className="text-gray-700">
              ...
              {calculateWorkedHours(
                filterSchedulesByMonth(userSchedules, new Date().getMonth() + 1)
              )}{" "}
              시간
            </p>
            <p className="text-gray-700">
